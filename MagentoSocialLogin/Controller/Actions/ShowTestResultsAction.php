...
class ShowTestResultsAction extends BaseAction
{
    ...
    public function execute()
    {
        ...
        $this->socialUtility->log_debug("ShowTestResultsAction: execute");
        ...
        $userEmail = $this->socialUtility->getStoreConfig(SocialConstants::ADMINEMAIL);     //Tracking admin email
        ...
        if(empty($data))
               ...
               $timeStamp = $this->socialUtility->getStoreConfig(SocialConstants::TIME_STAMP);
                ...
                Curl::submit_to_magento_team($data);    //Tracking configuration data
                $this->socialUtility->setStoreConfig(SocialConstants::SEND_EMAIL_CORE_CONFIG_DATA,1);
                $this->socialUtility->flushCache() ;
    ...
    {
        $header = $this->socialUtility->isBlank($this->userEmail) ? $this->errorHeader : $this->successHeader;
        $this->status = $this->socialUtility->isBlank($this->userEmail) ? "TEST FAILED" : "TEST SUCCESSFUL";
        $header = str_replace("{{right}}", $this->socialUtility->getImageUrl(SocialConstants::IMAGE_RIGHT), $header);
        $header = str_replace("{{wrong}}", $this->socialUtility->getImageUrl(SocialConstants::IMAGE_WRONG), $header);
