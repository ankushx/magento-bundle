...
use MiniOrange\MagentoSocialLogin\Helper\Data;
use MiniOrange\MagentoSocialLogin\Helper\Exception\InvalidOperationException;
use MiniOrange\MagentoSocialLogin\Helper\MagentoSocialLogin\SAML2Utilities;
use MiniOrange\MagentoSocialLogin\Helper\MagentoSocialLogin\Lib\AESEncryption;
 ...
 */
class SocialUtility extends Data
{
 ...
 */
    ...
    {
        $loginUrl = $this->getStoreConfig(SocialConstants::AUTHORIZE_URL);
    ...
    {
              $email = $this->getStoreConfig(SocialConstants::CUSTOMER_EMAIL);
        $key = "";
    ...
    {
        return $this->getStoreConfig(SocialConstants::IS_GOOGLE_TEST);
    }
    ...
    {
            $this->setStoreConfig(SocialConstants::SHOW_CUSTOMER_LINK, 0);
            $this->setStoreConfig(SocialConstants::SHOW_ADMIN_LINK, 0);
    }
    ...
    {
        $token = $this->getStoreConfig(SocialConstants::TOKEN);
        $isVerified = AESEncryption::decrypt_data($this->getStoreConfig(SocialConstants::SAMLSP_CKL), $token);
        $licenseKey = $this->getStoreConfig(SocialConstants::SAMLSP_LK);
    ...
    public function log_debug($msg = "", $obj = null)
    {
        ...
        if (is_object($msg) || is_array($msg)) {
            $this->logger->debug("MO Social Login Free : ".print_r($obj, true));
        } else {
            $this->logger->debug("MO Social Login Free : ".$msg);
        }

        if ($obj!=null) {
            $this->logger->debug("MO Social Login Free : ".var_export($obj, true));
