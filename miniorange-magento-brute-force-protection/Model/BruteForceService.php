...
class BruteForceService
{
    ...
    public function processLogin($subject, \Closure $proceed)
    {
        ...
        if ($request->isPost()) {
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                ...
                $maxAttemptsDelayConfig = $this->bruteforceutility->getStoreConfig('bruteforce/customer/max_attempts_delay', 'stores', $storeId);
                $maxAttemptsDelay = ($maxAttemptsDelayConfig !== null && $maxAttemptsDelayConfig !== '') ? (int)$maxAttemptsDelayConfig : 3;
                $maxAttemptsLockoutTemporaryConfig = $this->bruteforceutility->getStoreConfig('bruteforce/customer/max_attempts_lockout', 'stores', $storeId);
    ...
    public function checkBruteForceProtection($username, $storeId = null)
    {
        ...
        $enabled = $this->bruteforceutility->getStoreConfig('bruteforce/customer/enabled', 'stores', $storeId);
        $maxAttemptsDelayConfig = $this->bruteforceutility->getStoreConfig('bruteforce/customer/max_attempts_delay', 'stores', $storeId);
        $maxAttemptsDelay = ($maxAttemptsDelayConfig !== null && $maxAttemptsDelayConfig !== '') ? (int)$maxAttemptsDelayConfig : 3;
        $delaySeconds = $this->bruteforceutility->getStoreConfig('bruteforce/customer/delay_seconds', 'stores', $storeId) ?: 30;
        $maxAttemptsLockoutTemporaryConfig = $this->bruteforceutility->getStoreConfig('bruteforce/customer/max_attempts_lockout', 'stores', $storeId);
        $maxAttemptsLockoutTemporary = ($maxAttemptsLockoutTemporaryConfig !== null && $maxAttemptsLockoutTemporaryConfig !== '') ? (int)$maxAttemptsLockoutTemporaryConfig : 5;
        $lockoutDurationMinutes = $this->bruteforceutility->getStoreConfig('bruteforce/customer/lockout_duration_minutes', 'stores', $storeId) ?: 30;
        $maxAttemptsLockoutPermanentConfig = $this->bruteforceutility->getStoreConfig('bruteforce/customer/max_attempts_permanent_lockout', 'stores', $storeId);
        ...
        $EmailNotificationsEnabled = $this->bruteforceutility->getStoreConfig('bruteforce/customer/email_notifications_enabled', 'stores', $storeId);
        $emailNotificationTiming = $this->bruteforceutility->getStoreConfig('bruteforce/customer/email_notification_timing', 'stores', $storeId) ?: 'both';
        $AdminAlertEnabled = $this->bruteforceutility->getStoreConfig('bruteforce/customer/admin_alert_enabled', 'stores', $storeId);
        $customerEmailTemplateTemporary = $this->bruteforceutility->getStoreConfig('bruteforce/customer/customer_email_template_temporary', 'stores', $storeId);
        $customerEmailTemplatePermanent = $this->bruteforceutility->getStoreConfig('bruteforce/customer/customer_email_template_permanent', 'stores', $storeId);
            ...
            if ($shouldCheckTempLockout) {
                ...
                if (!$inTempLockoutList && $tempLockoutLimitExceeded) {
                    ...
                    if ($inDelayList || !$delayLimitExceeded) {
                        ...
                        $delaySeconds = (int)$this->bruteforceutility->getStoreConfig('bruteforce/customer/delay_seconds', 'stores', $storeId) ?: 30;
    ...
    public function checkTemporaryLockout($username, $customerId, $lockedAccountData = null, $storeId = null)
    {
        ...
        $lockoutDurationMinutes = (int)$this->bruteforceutility->getStoreConfig('bruteforce/customer/lockout_duration_minutes', 'stores', $storeId);
