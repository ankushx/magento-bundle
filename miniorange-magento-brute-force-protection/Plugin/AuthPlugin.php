...
class AuthPlugin
{
    ...
    public function aroundLogin(Auth $subject, \Closure $proceed, $username, $password)
    {
        ...
        try { 
            ...
            $maxAttemptsDelayConfig = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/max_attempts_delay', 'default', 0);
            $maxAttemptsDelay = ($maxAttemptsDelayConfig !== null && $maxAttemptsDelayConfig !== '') ? (int)$maxAttemptsDelayConfig : 3;
            $maxAttemptsLockoutTemporaryConfig = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/max_attempts_lockout', 'default', 0);
            ...
            if ($subject->getCredentialStorage()->getId()) {
                ...
                $enableBFP = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/enabled', 'default', 0);
    ...
    public function preCheckAdminLockoutAndDelay($username)
    {
        ...
        $maxAttemptsDelayConfig = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/max_attempts_delay', 'default', 0);
        $maxAttemptsDelay = ($maxAttemptsDelayConfig !== null && $maxAttemptsDelayConfig !== '') ? (int)$maxAttemptsDelayConfig : 3;
        $delaySeconds = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/delay_seconds', 'default', 0) ?: 30;
    ...
    public function checkBruteForceProtectionAdmin($username)
    {
        ...
        $maxAttemptsDelay = ($maxAttemptsDelayConfig !== null && $maxAttemptsDelayConfig !== '') ? (int)$maxAttemptsDelayConfig : 3;
        $delaySeconds = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/delay_seconds', 'default', 0) ?: 30;
        $maxAttemptsWarning = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/max_attempts_warning', 'default', 0) ?: 2;
        $maxAttemptsLockoutTemporaryConfig = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/max_attempts_lockout', 'default', 0);
        $maxAttemptsLockoutTemporary = ($maxAttemptsLockoutTemporaryConfig !== null && $maxAttemptsLockoutTemporaryConfig !== '') ? (int)$maxAttemptsLockoutTemporaryConfig : 5;
        $emailNotificationTiming = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/email_notification_timing', 'default', 0) ?: 'both';
        $adminEmailTemplateTemporary = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/admin_email_template_temporary', 'default', 0);
    ...
    protected function handleAdminAccountLockout($username, $adminEmailTemplateTemporary, $emailNotificationTiming)
    {
        ...
        $lockoutDurationMinutes = (int)$this->bruteForceUtility->getStoreConfig('bruteforce/admin/lockout_duration_minutes', 'default', 0);
    ...
    protected function sendTemporaryLockoutEmail($username, $adminEmailTemplate)
    {
        try {
            ...
            $emailNotificationsEnabled = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/email_notifications_enabled', 'default', 0);
    ...
    protected function createAdminAlert($username, $lockoutType = 'temporary')
    {
        try {
            ...
            // Admin BruteForce settings are always global (default scope)
            $adminAlertEnabled = $this->bruteForceUtility->getStoreConfig('bruteforce/admin/admin_alert_enabled', 'default', 0);
