...
class Index extends BaseAdminAction implements HttpPostActionInterface, HttpGetActionInterface
{
    ...
    protected function handleAutoSave($params)
    {
        ...
        if (isset($params['email_notifications_enabled'])) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/email_notifications_enabled', (int)$params['email_notifications_enabled'], $scope, $scopeId);
        }
        
        if (isset($params['admin_alert_enabled'])) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/admin_alert_enabled', (int)$params['admin_alert_enabled'], $scope, $scopeId);
        }
        
        if (isset($params['forgot_password_email_notifications_enabled'])) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_email_notifications_enabled', (int)$params['forgot_password_email_notifications_enabled'], $scope, $scopeId);
        }
        
        if (isset($params['forgot_password_admin_alert_enabled'])) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_admin_alert_enabled', (int)$params['forgot_password_admin_alert_enabled'], $scope, $scopeId);
        }
    ...
    protected function saveConfigurations($params)
    {
        ...
        if (isset($params['enabled'])) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/enabled', (int)$params['enabled'], $scope, $scopeId);
        } else {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/enabled', 0, $scope, $scopeId);
        }
        ...
        if (array_key_exists('max_attempts_delay', $params)) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/max_attempts_delay', (int)$params['max_attempts_delay'], $scope, $scopeId);
        }

        if (array_key_exists('delay_seconds', $params)) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/delay_seconds', (int)$params['delay_seconds'], $scope, $scopeId);
        }

        // Account Lockout Settings
        if (array_key_exists('max_attempts_lockout', $params)) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/max_attempts_lockout', (int)$params['max_attempts_lockout'], $scope, $scopeId);
        }

        if (array_key_exists('lockout_duration_minutes', $params)) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/lockout_duration_minutes', (int)$params['lockout_duration_minutes'], $scope, $scopeId);
        }

        if (array_key_exists('max_attempts_permanent_lockout', $params)) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/max_attempts_permanent_lockout', (int)$params['max_attempts_permanent_lockout'], $scope, $scopeId);
        }

        // TwoFA Integration
        // COMMENTED OUT: TwoFA Invoking settings saving
        // if (isset($params['max_attempts_twofa'])) {
        //     $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/max_attempts_twofa', (int)$params['max_attempts_twofa'], $scope, $scopeId);
        // }
        ...
        //     $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/only_invoke_twofa_on_failed_attempts', (int)$params['only_invoke_twofa_on_failed_attempts'], $scope, $scopeId);
        // } else {
        //     $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/only_invoke_twofa_on_failed_attempts', 0, $scope, $scopeId);
        // }
        ...
        if (isset($params['email_notifications_enabled'])) {
            ...
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/email_notifications_enabled', 0, $scope, $scopeId);
        }
        ...
        if (isset($params['admin_alert_enabled'])) {
            ...
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/admin_alert_enabled', 0, $scope, $scopeId);
        }

        // Email notification timing for both customer and admin
        if (isset($params['email_notification_timing'])) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/email_notification_timing', $params['email_notification_timing'], $scope, $scopeId);
        }

        // Customer email templates only for customer type
        if ($isCustomer) {
            if (isset($params['customer_email_template_temporary'])) {
                $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/customer_email_template_temporary', $params['customer_email_template_temporary'], $scope, $scopeId);
            }
            if (isset($params['customer_email_template_permanent'])) {
                $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/customer_email_template_permanent', $params['customer_email_template_permanent'], $scope, $scopeId);
            }
        }
        ...
        if (!$isCustomer) {
            if (isset($params['admin_email_template_temporary'])) {
                $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/admin_email_template_temporary', $params['admin_email_template_temporary'], $scope, $scopeId);
            }
            if (isset($params['admin_email_template_permanent'])) {
                $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/admin_email_template_permanent', $params['admin_email_template_permanent'], $scope, $scopeId);
            }
        }
        ...
        //         $this->bruteforceutility->setStoreConfig('bruteforce/admin/away_mode_enabled', (int)$params['away_mode_enabled'], 'default', 0);
        //     } else {
        //         $this->bruteforceutility->setStoreConfig('bruteforce/admin/away_mode_enabled', 0, 'default', 0);
        //     }
        //
        //     if (isset($params['away_mode_from_time'])) {
        //         $this->bruteforceutility->setStoreConfig('bruteforce/admin/away_mode_from_time', $params['away_mode_from_time'], 'default', 0);
        //     }
        //
        //     if (isset($params['away_mode_to_time'])) {
        //         $this->bruteforceutility->setStoreConfig('bruteforce/admin/away_mode_to_time', $params['away_mode_to_time'], 'default', 0);
        //     }
        //
        ...
        //         $this->bruteforceutility->setStoreConfig('bruteforce/admin/away_mode_days', $daysString, 'default', 0);
        //     } elseif (isset($params['away_mode_days'])) {
        //         // If it's already a string (from form submission)
        //         $this->bruteforceutility->setStoreConfig('bruteforce/admin/away_mode_days', $params['away_mode_days'], 'default', 0);
        //     } else {
        //         // Clear days if not provided
        //         $this->bruteforceutility->setStoreConfig('bruteforce/admin/away_mode_days', '', 'default', 0);
        //     }
        // }
        ...
        if (isset($params['forgot_password_enabled'])) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_enabled', (int)$params['forgot_password_enabled'], $scope, $scopeId);
        } else {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_enabled', 0, $scope, $scopeId);
        }

        // Forgot Password Custom Threshold for Delay
        // Use array_key_exists to ensure 0 values are saved
        if (array_key_exists('forgot_password_max_attempts_delay', $params)) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_max_attempts_delay', (int)$params['forgot_password_max_attempts_delay'], $scope, $scopeId);
        }

        if (array_key_exists('forgot_password_delay_seconds', $params)) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_delay_seconds', (int)$params['forgot_password_delay_seconds'], $scope, $scopeId);
        }

        // Forgot Password Account Lockout Settings
        if (array_key_exists('forgot_password_max_attempts_lockout', $params)) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_max_attempts_lockout', (int)$params['forgot_password_max_attempts_lockout'], $scope, $scopeId);
        }

        if (array_key_exists('forgot_password_lockout_duration_minutes', $params)) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_lockout_duration_minutes', (int)$params['forgot_password_lockout_duration_minutes'], $scope, $scopeId);
        }
        ...
        //     $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_max_attempts_twofa', (int)$params['forgot_password_max_attempts_twofa'], $scope, $scopeId);
        // }
        ...
        //     $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_invoke_twofa_logged_in', (int)$params['forgot_password_invoke_twofa_logged_in'], $scope, $scopeId);
        // } else {
        //     $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_invoke_twofa_logged_in', 0, $scope, $scopeId);
        // }
        ...
        if (isset($params['forgot_password_email_notifications_enabled'])) {
            ...
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_email_notifications_enabled', 0, $scope, $scopeId);
        }
        ...
        if (isset($params['forgot_password_admin_alert_enabled'])) {
            ...
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_admin_alert_enabled', 0, $scope, $scopeId);
        }

        // Admin email template only for admin type, not for customer
        if (!$isCustomer && isset($params['forgot_password_admin_email_template'])) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_admin_email_template', $params['forgot_password_admin_email_template'], $scope, $scopeId);
        }

        // Customer email template only for customer type
        if ($isCustomer && isset($params['forgot_password_customer_email_template'])) {
            $this->bruteforceutility->setStoreConfig('bruteforce/' . $type . '/forgot_password_customer_email_template', $params['forgot_password_customer_email_template'], $scope, $scopeId);
        }
