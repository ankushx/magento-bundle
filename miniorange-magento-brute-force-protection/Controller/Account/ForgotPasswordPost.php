...
class ForgotPasswordPost extends \Magento\Customer\Controller\Account\ForgotPasswordPost
{
    ...
    public function execute()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($email)) {
                ...
                $maxAttemptsDelayConfig = $this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_max_attempts_delay', 'stores', $storeId);
                $maxAttemptsDelay = ($maxAttemptsDelayConfig !== null && $maxAttemptsDelayConfig !== '') ? (int)$maxAttemptsDelayConfig : 3;
                $maxAttemptsLockoutTemporaryConfig = $this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_max_attempts_lockout', 'stores', $storeId);
    ...
    protected function checkBruteForceProtection($email)
    {
        ...
        $enabled = $this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_enabled', 'stores', $storeId);
        $maxAttemptsDelayConfig = $this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_max_attempts_delay', 'stores', $storeId);
        $maxAttemptsDelay = ($maxAttemptsDelayConfig !== null && $maxAttemptsDelayConfig !== '') ? (int)$maxAttemptsDelayConfig : 3;
        $delaySeconds = $this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_delay_seconds', 'stores', $storeId) ?: 30;
        $maxAttemptsLockoutTemporaryConfig = $this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_max_attempts_lockout', 'stores', $storeId);
        $maxAttemptsLockoutTemporary = ($maxAttemptsLockoutTemporaryConfig !== null && $maxAttemptsLockoutTemporaryConfig !== '') ? (int)$maxAttemptsLockoutTemporaryConfig : 5;
        $lockoutDurationMinutes = $this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_lockout_duration_minutes', 'stores', $storeId) ?: 30;
        $customerEmailTemplate = $this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_customer_email_template', 'stores', $storeId);
        ...
        if ($lockedAccountData['lock_type'] === 'temporary' && $lockedAccountData['first_time_lockout'] && $lockedAccountData['lock_until']) {
            ...
            if ($currentTime < $lockoutEndTime) {
                ...
                if ($thresholdParity === $nextAttemptParity) {
                    ...
                    $lockoutDurationMinutes = (int)$this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_lockout_duration_minutes', 'stores', $storeId);
    ...
    protected function createAdminAlert($email, $lockoutType = 'temporary')
    {
        try {
            ...
            // Get current store ID for store-specific configuration
            $storeId = $this->storeManager->getStore()->getId();
            $adminAlertEnabled = $this->bruteforceutility->getStoreConfig('bruteforce/customer/forgot_password_admin_alert_enabled', 'stores', $storeId);
