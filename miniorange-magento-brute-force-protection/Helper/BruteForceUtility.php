...
class BruteForceUtility
{
    ...
    public function isAwayModeActive()
    {
        ...
        $awayModeEnabled = $this->getStoreConfig('bruteforce/admin/away_mode_enabled', 'default', 0);
        if (!$awayModeEnabled) {
            return false; // Away mode is disabled, allow access
        }

        // Get away mode settings
        $fromTime = $this->getStoreConfig('bruteforce/admin/away_mode_from_time', 'default', 0) ?: '00:00:00';
        $toTime = $this->getStoreConfig('bruteforce/admin/away_mode_to_time', 'default', 0) ?: '23:59:59';
        $daysString = $this->getStoreConfig('bruteforce/admin/away_mode_days', 'default', 0) ?: '';
    ...
    public function getTempLockoutLimitConfig($context = 'customer')
    {
        try {
            ...
            $userIdsPath = 'bruteforce/temp_lockout/customer_ids_encrypted';
            $limitPath = 'bruteforce/temp_lockout/max_limit_encrypted';
            $tokenPath = 'bruteforce/temp_lockout/encryption_token';
            
            if ($context === 'admin') {
                $userIdsPath = 'bruteforce/admin_temp_lockout/user_ids_encrypted';
            } elseif ($context === 'admin_forgot_password') {
                $userIdsPath = 'bruteforce/admin_temp_lockout/forgot_user_ids';
            } elseif ($context === 'forgot_password') {
                $userIdsPath = 'bruteforce/temp_lockout/forgot_customer_ids';
            } elseif ($context === 'customer_delay') {
                $userIdsPath = 'bruteforce/delay/customer_ids_encrypted';
                $limitPath = 'bruteforce/delay/max_limit_encrypted';
                $tokenPath = 'bruteforce/delay/encryption_token';
            } elseif ($context === 'admin_delay') {
                $userIdsPath = 'bruteforce/admin_delay/user_ids_encrypted';
                $limitPath = 'bruteforce/delay/max_limit_encrypted';
                $tokenPath = 'bruteforce/delay/encryption_token';
            } elseif ($context === 'forgot_password_delay') {
                $userIdsPath = 'bruteforce/delay/forgot_customer_ids';
                $limitPath = 'bruteforce/delay/max_limit_encrypted';
                $tokenPath = 'bruteforce/delay/encryption_token';
            } elseif ($context === 'admin_forgot_password_delay') {
                $userIdsPath = 'bruteforce/admin_delay/forgot_user_ids';
                $limitPath = 'bruteforce/delay/max_limit_encrypted';
                $tokenPath = 'bruteforce/delay/encryption_token';
            }
    ...
    public function getNotificationLimitConfig($type, $context, $action)
    {
        try {
            ...
            $token = $this->getStoreConfig('bruteforce/temp_lockout/encryption_token', 'default', 0);
            if (!$token) {
                $token = $defaultToken;
            }
    ...
    protected function sendLimitExceededTracking($limitType)
    {
        try {
            ...
            $limitExceededSent = $this->getStoreConfig('bruteforce/tracking/limit_exceeded_sent', 'default', 0);
            ...
            if ($adminUser) {
                ...
                if ($adminEmail) {
                    $this->setStoreConfig('bruteforce/tracking/admin_email', $adminEmail, 'default', 0);
                }
            }
            
            // If no admin session, try to get stored admin email from DB
            if (empty($adminEmail)) {
                $adminEmail = $this->getStoreConfig('bruteforce/tracking/admin_email', 'default', 0);
            }
            ...
            // Get timestamp (check if dataadded is set, if yes use existing timestamp, else generate new)
            $dataAdded = $this->getStoreConfig('bruteforce/tracking/dataadded', 'default', 0);
            $timeStamp = $this->getStoreConfig('bruteforce/tracking/timestamp', 'default', 0);
            ...
            if (!$timeStamp || $dataAdded == 0) {
                ...
                $this->setStoreConfig('bruteforce/tracking/timestamp', $timeStamp, 'default', 0);
            }
            ...
            $this->setStoreConfig('bruteforce/tracking/limit_exceeded_sent', 1, 'default', 0);
        } catch (\Exception $e) {
