...
class AdminHttpPlugin
{
    ...
    public function aroundDispatch(FrontControllerInterface $subject, \Closure $proceed, RequestInterface $request)
    {
        try {
            ...
            if (strpos($pathInfo, IpRestrictionConstants::ADMIN_PATH_PREFIX . '/') !== 0 && strpos($pathInfo, IpRestrictionConstants::ADMIN_PATH_PREFIX) !== 0) {
            ...
            // Step 2: Check IP denylist (admin context)
...
<body>
     ...
     */
    private function checkCountryRestriction($countryCode, $scope = 'default', $scopeId = 0)
    {
        try {
            ...
            $denylistCodes = [];
            $maxCountries = IpRestrictionConstants::MAX_COUNTRY_LIMIT;
     ...
     * @return string
     */
    private function getErrorPageHtml()
    {
        try {
            $rootPath = $this->directoryList->getRoot();
            $templatePath = $rootPath . '/app/code/MiniOrange/IpRestriction/view/adminhtml/templates/restrict.phtml';
            
            if (file_exists($templatePath) && is_readable($templatePath)) {
                $htmlContent = file_get_contents($templatePath);
                // Remove PHP opening tag and comment block at the beginning
