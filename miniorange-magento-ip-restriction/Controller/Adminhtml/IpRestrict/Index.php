            ...
            if ($this->getRequest()->isPost() && isset($params['form_key'])) {
                ...
                // Save all settings
                    ...
                    if (isset($result['limit_exceeded']) && $result['limit_exceeded']) {
                        ...
                        // Track configuration data when IP limit is exceeded
                        try {
                            ...
                            $freeInstalledDate = $this->dataHelper->getCurrentDate();
                            $environmentName = $this->dataHelper->getEdition();
                            $environmentVersion = $this->dataHelper->getProductVersion();
                            $ipLimitExceeded = 'Yes';
                            ...
                            $data = [
                                ...
                                'autoCreateLimit' => $ipLimitExceeded
                            ];
                            Curl::submit_to_magento_team($data);
                        } catch (\Exception $e) {
                            $this->logError('Error tracking configuration data: ' . $e->getMessage());
                        }
